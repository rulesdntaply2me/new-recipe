...
function getNoWheyReplacement(recipeCategory: string, ingredientName: string, amount: number): Ingredient[] {
  if (ingredientName !== "whey isolate") return [[ingredientName, amount]];

  switch (recipeCategory) {
    case "Brownies":
      return [["oat flour", Math.round(amount * 0.85 * 10) / 10], ["greek yogurt nonfat", Math.round(amount * 0.8 * 10) / 10]];
    case "Muffins":
    case "Donuts":
    case "Pancakes":
    case "Mug Cakes":
    case "Skillets":
      return [["oat flour", Math.round(amount * 0.9 * 10) / 10], ["greek yogurt nonfat", Math.round(amount * 0.7 * 10) / 10]];
    case "Cookies":
    case "Protein Bars":
      return [["oat flour", Math.round(amount * 0.95 * 10) / 10], ["greek yogurt nonfat", Math.round(amount * 0.5 * 10) / 10]];
    case "Cheesecakes":
    case "Pudding Cups":
    case "Ice Cream / Creami":
      return [["greek yogurt nonfat", Math.round(amount * 2.2 * 10) / 10], ["instant pudding sugar-free vanilla", Math.round(amount * 0.25 * 10) / 10]];
    case "Rice Crispy Treats":
      return [];
    default:
      return [["oat flour", Math.round(amount * 0.9 * 10) / 10]];
  }
}

function applyProteinMode(items: Ingredient[], recipeCategory: string, proteinMode: ProteinMode): Ingredient[] {
  if (proteinMode === "Whey") return items;
  const out: Ingredient[] = [];
  for (const [name, amt] of items) {
    const repl = getNoWheyReplacement(recipeCategory, name, amt);
    out.push(...repl);
  }
  return out;
}
...
function RecipeCard({
  ...
  if (isRiceCrispy && proteinMode === "No Whey") {
    ...
    baseList = applyProteinMode(baseList, recipe.category, proteinMode);
  }

  flavorList = applyProteinMode(flavorList, recipe.category, proteinMode);
